...
export class BillingService {
    ...
    async createCheckoutSession(
        ...
        try {
            session = await this.stripe.checkout.sessions.create({
                ...
                line_items: [
                    { price: tierData[priceField], quantity: 1 },
                ],
                metadata: {
                    user_id: userId,
                    tier: tier,
    ...
    async createAddonCheckout(
        ...
        try {
            session = await this.stripe.checkout.sessions.create({
                ...
                metadata: { user_id: userId, addon_type: 'credits_100' },
                payment_intent_data: {
                    metadata: { user_id: userId, addon_type: 'credits_100' },
                },
    ...
    async getPlans() {
        const { data, error } = await this.databaseService.client
            .from('subscription_tiers')
            .select('tier_key, display_name, monthly_price_cents, annual_price_cents, credits_per_month, max_brands, max_products_per_brand, features, display_order')
            .eq('is_active', true)
    ...
    async verifyCheckoutSession(userId: string): Promise<{
        ...
        const { error: subError } = await this.databaseService.client
            .from('subscriptions')
            .upsert({
                user_id: userId,
    ...
    private async onCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
        ...
        if (session.metadata?.addon_type === 'canva_template') {
            ...
            try {
                ...
                await this.canvaService.createOrder(userId, session.customer_details?.email || '', session.customer_details?.name || null, {
        ...
        const { error: subUpsertError } = await this.databaseService.client
            .from('subscriptions')
            .upsert({
                user_id: userId,
