...
export class GeminiService {
	...
	private buildProductFrontLabel(idx: number): string {
		return [
			...
			'  ═══════════════════════════════════════════',
			'',
			'  ═══ PRODUCT FIDELITY — ZERO EXTRA ELEMENTS (CRITICAL) ═══',
			'  • Draw ONLY what exists in the product reference photo — NOTHING MORE',
			'  • If the photo shows a bottle, draw ONLY that bottle — no extra bottles, no extra objects',
			'  • If the photo shows a box/packaging, draw the box EXACTLY as it appears — do NOT remove or alter it',
			'  • Do NOT add: figurines, mascots, animals, plants, decorative objects, extra product units',
			'  • Do NOT add: floating ingredients, splashes, powder explosions, smoke effects around the product',
			'  • Do NOT duplicate the product — if the photo shows 1 item, render exactly 1 item',
			'  • Do NOT "improve" or "enhance" the product appearance — reproduce it AS-IS from the reference',
			'  • Do NOT add props, accessories, or context objects that are NOT in the reference photo',
			'  • The product in the ad must be a FAITHFUL reproduction of the reference photo, not an idealized version',
			'  • RULE: If it is NOT visible in the product reference photo, it must NOT appear in the ad',
			'  ═══════════════════════════════════════════════════════════',
		].join('\n');
	}
